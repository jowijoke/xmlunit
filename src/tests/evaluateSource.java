...
public class evaluateSource {
	...
	public void diffSource() {
		// test that ignores specific attributes in order to pass but diff.xml has aggregated which includes extra map keys to ignore
				Diff myDiff3 = DiffBuilder.compare(Input.fromFile("xml/Rule/clean_complete_email_rule.xml"))
			            ...
			            .build();
				if(myDiff3.hasDifferences()) {
					Iterable<Difference> result = myDiff3.getDifferences();
					for (Difference d : result) {
						System.out.println("d: " + d.toString(new IIQComparisonFormatter()));
					}
				}
	...
	public void test6() {
		...
		String local = "import java.util.*;\n" + 
				...
				"              }\n" + 
				"          }";
		String control = "import java.util.*;\n" + 
				"          import sailpoint.tools.Util;\n" + 
				"          import sailpoint.tools.GeneralException;\n" + 
				"          import sailpoint.object.Configuration;\n" + 
				"          import sailpoint.object.EmailOptions;\n" + 
				...
				"              }\n" + 
				"          }";
		Diff myDiff5 = DiffBuilder.compare(Input.fromString(control))
	            .withTest(Input.fromString(local))
	            .ignoreComments()
	            .ignoreWhitespace()
