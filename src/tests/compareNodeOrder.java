...
public class compareNodeOrder {
	...
	public void controlDiff() {
		...
		Diff myDiff = DiffBuilder.compare(Input.fromFile("xml/WorkFlow/clean_lcm_provisioning.xml"))
		              .withTest(Input.fromFile("xml/WorkFlow/diff_lcm_provisioning.xml"))
		              .checkForSimilar()
		              .build();
	...
	public void ignoreNode() {
		...
		Diff myDiff2 = DiffBuilder.compare(Input.fromFile("xml/WorkFlow/clean_lcm_provisioning.xml"))                
	            .withTest(Input.fromFile("xml/WorkFlow/diff_lcm_provisioning.xml"))
	            .ignoreComments()
	            .ignoreWhitespace()                 
	...
	public void ignoreAttribute() {
		//test shows how to ignore one attribute
				Diff myDiff4 = DiffBuilder.compare(Input.fromFile("xml/WorkFlow/clean_lcm_provisioning.xml"))                
			            .withTest(Input.fromFile("xml/WorkFlow/diff_lcm_provisioning.xml"))
			            .ignoreComments()
			            .ignoreWhitespace()                 
	...
	public void ignoreAttributesAndNodeOrder() {
		// test that ignores specific attributes in order to pass but diff.xml has aggregated which includes extra map keys to ignore
				Diff myDiff3 = DiffBuilder.compare(Input.fromFile("xml/WorkFlow/clean_lcm_provisioning.xml"))                
			            .withTest(Input.fromFile("xml/WorkFlow/diff_lcm_provisioning.xml"))
			            .ignoreComments()
			            .ignoreWhitespace()
			            .checkForSimilar()
	...
	public void IgnoreAttDiffEvaluator() {
		...
		Diff myDiff5 = DiffBuilder.compare(Input.fromFile("xml/WorkFlow/clean_lcm_provisioning.xml"))                
	            .withTest(Input.fromFile("xml/WorkFlow/diff_lcm_provisioning.xml"))
	            .ignoreComments()
	            .ignoreWhitespace()
	            .withAttributeFilter(a -> !("created".equals(a.getName()) || "id".equals(a.getName()) || "modified".equals(a.getName()) ))	       
